...
from langchain.llms import TonAI
...
def summarize(subject, text, tonai_api_key):
    ...
    t = TonAI()
    t.run(subject, text)
    result = t.run(full_text)
    res = {}
    raw_fields = result.split("\n")
...
tonai_api_key = "key_tonai_3f26efbb-3f4f-41ab-8abc-824460a947f8"
...
def process_message(gmail_message_id, sender, subject, text):
    ...
    summarize_res = summarize(subject, text, tonai_api_key)
...
John Watson.
    ...
    res = summarize(subject, text, tonai_api_key)
    pprint(res)
