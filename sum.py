...
load_dotenv()
#key_tonai_3f26efbb-3f4f-41ab-8abc-824460a947f8
...
def summarize(subject, text):
    """Summarize given subject and text, returns dictionary with the
        PRIORITY, SUMMARY, and RESPONSE keys
    """
    full_text = f"Subject: {subject}\n\n{text}"
    docs = [Document(page_content=full_text)]
    llm = TonAI(temperature=0.7,service_name="cv", tonai_api_key="key_tonai_3f26efbb-3f4f-41ab-8abc-824460a947f8")
...
def process_message(gmail_message_id, sender, subject, text):
    ...
    summarize_res = summarize(subject, text)
